...
module.exports.load = (gulp, config) => {
  ...
  gulp.task('ieconfig', () =>
    gulp
      .src(path.resolve(config.get('imagesPath'), 'favicons', 'ieconfig.xml'))
      .pipe(gulp.dest(path.resolve(config.get('imagesBuildPath'), 'favicons'))))

  gulp.task('build-images', () =>
    buildImages(gulp.src(config.get('imagesTmpOptimizedGlob')), config)
      .pipe(gulp.dest(config.get('imagesBuildPath'))))

  gulp.task('images', sequence('clean-images', 'optimize-images', 'build-images', 'ieconfig'))
